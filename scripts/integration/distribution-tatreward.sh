...
DISTRIBUTION_TX=$($BIN tx distribution withdraw-rewards  $VALIDATOR_ADDRESS --from $KEY1 --keyring-backend $KEYRING --keyring-dir ~/.treasurenetd --node $RPC_ENDPOINT --chain-id $CHAIN_ID --gas auto --fees 1unit --tatreward --yes)

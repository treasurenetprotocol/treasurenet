...
// bid v1.0.0
// func (k Keeper) NewApplyAndReturnValidatorSetUpdatestest(ctx sdk.Context, log sdk.ABCIMessageLogs) (updates []abci.ValidatorUpdate, err error) {
// 	params := k.GetParams(ctx)
// 	// var Mine []string
...
// 			}
// 			fmt.Printf("11111111111111111111bid功能中重新生成新的newTatiteratorlist:%+v\n", newTatiteratorlist)
// 			for index := range newTatiteratorlist {
// 				_, is := last[index]
// 				if !is {
// 					newvalAddrStr, _ := sdk.ValAddressFromBech32(index)
// 					// newval, _ := k.GetValidator(ctx, newvalAddrStr)
// 					// fmt.Printf("2222222newval:%+v\n", newval)
// 					newValidator := k.mustGetValidator(ctx, newvalAddrStr)
// 					fmt.Printf("333333333newValidator:%+v\n", newValidator)
// 					newPowerup := newValidator.ConsensusBidPower(powerReduction)
// 					fmt.Printf("44444444newPowerup:%+v\n", newPowerup)
// 					updates = append(updates, newValidator.ABCIBidValidatorUpdate(powerReduction))
// 					k.SetLastValidatorPower(ctx, newvalAddrStr, newPowerup)
// 					if newValidator.IsUnbonded() {
// 						newValidator, err = k.unbondedToBonded(ctx, newValidator)
// 						if err != nil {
// 							fmt.Println("测试步骤", 123)
// 							return
// 						}
// 						amtFromNotBondedToBonded = amtFromNotBondedToBonded.Add(newValidator.GetTokens())
// 					}
// 					if newValidator.IsUnbonding() {
// 						newValidator, err = k.unbondingToBonded(ctx, newValidator)
// 						if err != nil {
// 							fmt.Println("测试步骤", 123)
// 							return
// 						}
...
// 				}
// 			}
// 			Newiterator := k.ValidatorsPowerStoreIterator(ctx)
...
// 			for count := 0; Newiterator.Valid() && count < int(maxValidators); Newiterator.Next() {
// 				// everything that is iterated in this loop is becoming or already a
// 				// part of the bonded validator set
// 				// fmt.Printf("iterator.Value:%v\n", iterator.Value())
// 				valAddr := sdk.ValAddress(Newiterator.Value())
...
// 				// }
// 				valAddrStr, err := sdk.Bech32ifyAddressBytes(sdk.GetConfig().GetBech32ValidatorAddrPrefix(), valAddr)
// 				fmt.Println("valAddrStr1:", valAddrStr)
...
// 				totalPower = totalPower.Add(sdk.NewInt(newPower))
// 				fmt.Println("bid后开始监听生成新的Iterator:", totalPower)
// 			}
...
// 				newTatiteratorlist[valAddrStr] = valAddrStr
// 			}
// 			fmt.Printf("222222222bid功能中重新生成新的newTatiteratorlist:%+v\n", newTatiteratorlist)
// 			iterator := k.ValidatorsPowerStoreIterator(ctx)
// 			defer iterator.Close()
// 			fmt.Println("维持之前的iterator:", iterator)
...
// 				totalPower = totalPower.Add(sdk.NewInt(newPower))
// 				fmt.Println("totalPowerold(没有开始bid,按照之前的iterator,totalPower的值):", totalPower)
// 			}
// 			noLongerBonded, err := sortNoLongerBonded(last)
// 			if err != nil {
// 				return nil, err
...
// 						totalPower = totalPower.Add(sdk.NewInt(newPower))
// 					}
// 				} else if validator.Status == 3 {
// 					_, found := newlist[valAddrStr]
// 					if found {
// 						oldPowerBytes, _ := last[valAddrStr]
// 						if !bytes.Equal(oldPowerBytes, newPowerBytes) {
// 							updates = append(updates, validator.ABCIValidatorUpdate(powerReduction))
...
// 			default: // equal amounts of tokens; no update required
// 			}
// 			fmt.Printf("11111111111111111111bid功能中重新定义active validator totalPower:%+v\n", totalPower)
// 		} else if eventlog.MsgIndex == 2 {
// 			newTatiteratorlist := make(map[string]string)
// 			NewTatiterator := k.ValidatorsNewPowerStoreIterator(ctx)
// 			defer NewTatiterator.Close()
...
// 			// if len(Mine) == 0 {
// 			// TatTestIterator := k.ValidatorsPowerStoreIterator(ctx)
// 			// fmt.Println("TatTestIterator.Valid():", TatTestIterator.Valid())
// 			TatIterator := k.ValidatorsNewPowerStoreIterator(ctx)
// 			fmt.Println("TatIterator:", TatIterator)
// 			// iterator := k.ValidatorsNewPowerStoreIterator(ctx)
// 			// fmt.Println("TatIterator.Valid():", TatIterator.Valid())
// 			defer TatIterator.Close()
// 			// fmt.Println("TatIterator.Valid():", TatIterator.Valid())
// 			for count := 0; TatIterator.Valid() && count < int(maxValidators); TatIterator.Next() {
// 				valAddr := sdk.ValAddress(TatIterator.Value())
// 				validator := k.mustGetValidator(ctx, valAddr)
// 				// fmt.Printf("TatIterator validator:%+v\n", validator)
// 				if validator.Jailed {
// 					k.DeleteValidatorByTatPowerIndex(ctx, validator)
// 				}
// 				//
// 				// k.DeleteValidatorByTatPowerIndex2(ctx, validator)
// 			}
// 			Tat2Iterator := k.ValidatorsNewPowerStoreIterator(ctx)
// 			defer Tat2Iterator.Close()
// 			// fmt.Println("Tat2Iterator:", Tat2Iterator)
// 			// fmt.Println("Tat2Iterator.Valid():", Tat2Iterator.Valid())
// 			// for count := 0; count < int(maxValidators); Tat2Iterator.Next() {
// 			for count := 0; Tat2Iterator.Valid() && count < int(maxValidators); Tat2Iterator.Next() {
// 				// everything that is iterated in this loop is becoming or already a
// 				// part of the bonded validator set
// 				valAddr := sdk.ValAddress(Tat2Iterator.Value())
// 				validator := k.mustGetValidator(ctx, valAddr)
// 				fmt.Printf("validator:%+v\n", validator)
// 				if validator.Jailed {
// 					panic("should never retrieve a jailed validator from the power store")
...
func SelectList(listsupervalidator []string, listvalidator []string, req abci.RequestEndBlock) []string {
	...
	fmt.Println("SelectList SubtrDemo listval:", listval)
	...
	case len(listvalidator) >= 8 && len(listvalidator) < 200:
		Activelent = len(listvalidator) / 2
		fmt.Println("Activelent大于8小于200", Activelent)
	// case len(listvalidator) >= 5 && len(listvalidator) < 200:
	// 	Activelent = int(4)
	// 	fmt.Println("Activelent大于8小于200", Activelent)
	default:
		Activelent = len(listvalidator)
	...
	// switch {
	// case len(listsupervalidator) >= 100 && len(listval) >= 100:
	// 	newlistsupervalidator = listsupervalidator[:100]
	// 	newlistvalidator = listval[:100]
	// 	newvalidator = append(newlistsupervalidator, newlistvalidator...)
	// case len(listsupervalidator) < 100 && len(listval) < 100:
...
func (k Keeper) DeleteNewIterator(ctx sdk.Context) {
	...
	for ; TatIterator.Valid(); TatIterator.Next() {
		...
		// fmt.Printf("iterator.Value:%v\n", iterator.Value())
		valAddr := sdk.ValAddress(TatIterator.Value())
		fmt.Println("DeleteNewIterator1111111111111111111111:", valAddr)
		// fmt.Println("valAddr:", valAddr)
		validator := k.mustGetValidator(ctx, valAddr)
...
func (k Keeper) DeletsNewItreator2(ctx sdk.Context, maxValidators uint32, newselectlist []string) {
	...
	fmt.Printf("重新更改bid的集合1newlist:%+v\n", newlist)
	NewTatiterator := k.ValidatorsNewPowerStoreIterator(ctx)
	defer NewTatiterator.Close()
	for count := 0; NewTatiterator.Valid() && count < int(maxValidators); NewTatiterator.Next() {
		// everything that is iterated in this loop is becoming or already a
		...
		valAddr := sdk.ValAddress(NewTatiterator.Value())
		validator := k.mustGetValidator(ctx, valAddr)
		fmt.Printf("validator666666666:%+v\n", validator)
		// fetch the old power bytes
		valAddrStr, _ := sdk.Bech32ifyAddressBytes(sdk.GetConfig().GetBech32ValidatorAddrPrefix(), valAddr)
		fmt.Println("valAddrStr7777777:", valAddrStr)
		newTatiteratorlist[valAddrStr] = valAddrStr
		if len(newselectlist) > 0 {
			fmt.Println("newselectlist8888888:", newselectlist)
			_, found := newlist[valAddrStr]
			if !found {
				fmt.Println("valAddrStr10101010:", found)
	...
	fmt.Printf("重新更改bid的集合2newTatiteratorlist:%+v\n", newTatiteratorlist)
	for _, value := range newTatiteratorlist {
		delete(newlist, value)
	}
	if len(newlist) > 0 {
		fmt.Printf("重新更改bid的集合3newlist:%+v\n", newlist)

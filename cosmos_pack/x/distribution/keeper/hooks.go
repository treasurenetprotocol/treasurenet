...
// initialize validator distribution record 每次有验证者创建就应当为其创建并初始化validatorcurrentrewards(每个验证者当前时期的收益信息) 和 validatorshistoricalrewards(每个验证者历史时期的收益信息) 这两个值
func (h Hooks) AfterValidatorCreated(ctx sdk.Context, valAddr sdk.ValAddress) {
	val := h.k.stakingKeeper.Validator(ctx, valAddr)
	h.k.initializeValidator(ctx, val) //内部通过initializeValidator来创建并初始化当前和历史时期的收益信息
}

...
type (
	...
	ValidatorHistoricalRewards struct {   //用来统计还有多少未提取的收益，当ReferenceCount为0时就可以从链上删除该历史时期的收益信息
		CumulativeRewardRatio sdk.DecCoins `json:"cumulative_reward_ratio"`
		ReferenceCount        uint16       `json:"reference_count"`
	}

	ValidatorCurrentRewards struct {   //当前时期的收益信息
		Rewards sdk.DecCoins `json:"rewards"`
		Period  uint64       `json:"period"`
	}

	ValidatorCurrentRewardsRecord struct {
		ValidatorAddress sdk.ValAddress          `json:"validator_address"` //当前奖励
		Rewards          ValidatorCurrentRewards `json:"rewards"`           //当前时期
	}

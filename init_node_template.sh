...
rm -rf {{DATA_PATH}}/*
rm -rf {{DATA_PATH}}/.*
rm -rf {{HOME_PATH}}/*  # 清理默认目录

# Ensure necessary directories exist
mkdir -p {{DATA_PATH}}/{{PROJECT_NAME}}
mkdir -p {{DATA_PATH}}/.{{PROJECT_NAME}}
 sudo chown -R $USER:$USER {{DATA_PATH}}/{{PROJECT_NAME}}
 sudo chown -R $USER:$USER {{DATA_PATH}}/.{{PROJECT_NAME}}
...
$BIN keys add $KEY1 --keyring-backend $KEYRING --algo $KEYALGO 2>> {{DATA_PATH}}/{{PROJECT_NAME}}/validator0-phrases
$BIN keys add $KEY2 --keyring-backend $KEYRING --algo $KEYALGO 2>> {{DATA_PATH}}/{{PROJECT_NAME}}/orchestrator0-phrases
$BIN eth_keys add --keyring-backend $KEYRING >> {{DATA_PATH}}/{{PROJECT_NAME}}/validator0-eth-keys
...
VALIDATOR_KEY=$($BIN keys show validator0 -a $ARGS)
ORCHESTRATOR_KEY=$($BIN keys show orchestrator0 -a $ARGS)
$BIN add-genesis-account $ARGS $VALIDATOR_KEY $ALLOCATION
$BIN add-genesis-account $ARGS $ORCHESTRATOR_KEY $ALLOCATION

# Generate transaction
ETHEREUM_KEY=$(grep address {{DATA_PATH}}/{{PROJECT_NAME}}/validator0-eth-keys | sed -n "1"p | sed 's/.*://')
echo $ETHEREUM_KEY

$BIN gentx $ARGS --moniker $MONIKER --chain-id=$CHAIN_ID validator0 258000000000000000000aunit $ETHEREUM_KEY $ORCHESTRATOR_KEY

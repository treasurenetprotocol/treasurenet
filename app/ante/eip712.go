...
var treasurenetCodec codec.ProtoCodecMarshaler

func init() {
	registry := codectypes.NewInterfaceRegistry()
	treasurenet.RegisterInterfaces(registry)
	treasurenetCodec = codec.NewProtoCodec(registry)
}
...
func VerifySignature(
	...
	case *signing.SingleSignatureData:
		...
		signerChainID, err := treasurenet.ParseChainID(signerData.ChainID)
		if err != nil {
		...
		var optIface treasurenet.ExtensionOptionsWeb3TxI

		if err := treasurenetCodec.UnpackAny(opts[0], &optIface); err != nil {
			return sdkerrors.Wrap(err, "failed to proto-unpack ExtensionOptionsWeb3Tx")
		}

		extOpt, ok := optIface.(*treasurenet.ExtensionOptionsWeb3Tx)
		if !ok {
		...
		typedData, err := eip712.WrapTxToTypedData(treasurenetCodec, extOpt.TypedDataChainID, msgs[0], txBytes, feeDelegation)
		if err != nil {
		...
		if err != nil {
			return sdkerrors.Wrap(err, "Ecrecover failed")
		}

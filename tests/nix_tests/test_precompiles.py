...
from .network import Treasurenet
from .utils import (
    ADDRS,
    CONTRACTS,
    KEYS,
...
def ibc(request, tmp_path_factory):
    ...
    network = prepare_network(path, name, [treasurenet_build, "chainmain"])
    yield from network
...
def test_ibc_transfer(ibc):
    ...
    old_dst_balance = get_balance(ibc.chains["chainmain"], dst_addr, TREASURENET_IBC_DENOM)

    pc = get_precompile_contract(ibc.chains["treasurenet"].w3, "ICS20I")
    ...
    def check_balance_change():
        ...
        new_dst_balance = get_balance(
            ibc.chains["chainmain"], dst_addr, TREASURENET_IBC_DENOM
        )
...
def test_staking(ibc):
    ...
    w3 = treasurenet.w3
    amt = 1000000
    cli = treasurenet.cosmos_cli()
    del_addr = cli.address("signer2")
    src_denom = "aunit"
    validator_addr = cli.validators()[0]["operator_address"]

    old_src_balance = get_balance(treasurenet, del_addr, src_denom)
    ...
    tx = pc.functions.delegate(ADDRS["signer2"], validator_addr, amt).build_transaction(
        {"from": ADDRS["signer2"], "gasPrice": treasurenet_gas_price}
    )
    ...
    new_src_balance = get_balance(treasurenet, del_addr, src_denom)
    assert old_src_balance - amt - fee == new_src_balance
...
def test_staking_via_sc(ibc):
    ...
    approve_tx = contract.functions.testApprove(
        ...
        {"from": ADDRS["signer1"], "gasPrice": treasurenet_gas_price, "gas": 60000}
    )

    gas_estimation = treasurenet.w3.eth.estimate_gas(approve_tx)
    ...
    delegate_tx = contract.functions.testDelegate(
        ...
        {"from": ADDRS["signer1"], "gasPrice": treasurenet_gas_price, "gas": 180000}
    )
    gas_estimation = treasurenet.w3.eth.estimate_gas(delegate_tx)

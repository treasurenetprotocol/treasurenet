...
EVMOS_ADDRESS_PREFIX = "treasurenet"
DEFAULT_DENOM = "aunit"
# WEVMOS_ADDRESS = Web3.toChecksumAddress("0xcc491f589b45d4a3c679016195b3fb87d7848210")
WEVMOS_ADDRESS = Web3.to_checksum_address("0xcc491f589b45d4a3c679016195b3fb87d7848210")
TEST_CONTRACTS = {
...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:treasurenet_5005-1-node{i}":
            ini[section].update(
                {
...
def update_treasurenetd_and_setup_stride(modified_bin):
    def inner(path, base_port, config):
        update_treasurenet_bin(modified_bin)(path, base_port, config)
        setup_stride()(path, base_port, config)

    return inner


def update_treasurenet_bin(modified_bin, nodes=[0, 1]):
    """
    updates the treasurenet binary with a patched binary.

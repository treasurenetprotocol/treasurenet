...
DEFAULT_CHAIN_BINARY = "treasurenetd"
...
class Treasurenet:
    ...
    def copy(self):
        return Treasurenet(self.base_dir)
...
def setup_treasurenet(path, base_port, long_timeout_commit=False):
    ...
    yield from setup_custom_treasurenet(path, base_port, cfg)
...
def create_snapshots_dir(path, base_port, config, n_nodes=2):
    for idx in range(n_nodes):
        data_snapshots_dir = path / "treasurenet_5005-1" / f"node{idx}" / "data" / "snapshots"
        os.makedirs(data_snapshots_dir, exist_ok=True)


def setup_treasurenet_rocksdb(path, base_port, long_timeout_commit=False):
    """
    setup_treasurenet_rocksdb returns an Treasurenet chain compiled with RocksDB
...
def setup_custom_treasurenet(
    ...
    try:
        ...
        yield Treasurenet(
            path / "treasurenet_5005-1", chain_binary=chain_binary or DEFAULT_CHAIN_BINARY
        )
    finally:
        os.killpg(os.getpgid(proc.pid), signal.SIGTERM)
        proc.wait()
...
def build_patched_treasurenetd(patch_nix_file):
    """
    build the binary modified for a custom scenario
    e.g. allow to register WTreasurenet token

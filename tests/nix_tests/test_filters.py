...
from .network import setup_custom_treasurenet, setup_treasurenet
from .utils import (
    ADDRS,
    CONTRACTS,
    deploy_contract,
...
def custom_treasurenet(tmp_path_factory):
    ...
    yield from setup_treasurenet(path, 26200)


@pytest.fixture(scope="module")
def treasurenet_indexer(tmp_path_factory):
...
@pytest.fixture(
    scope="module",
    params=["treasurenet", "geth", "treasurenet-ws", "enable-indexer", "treasurenet-rocksdb"],
)
def cluster(request, custom_treasurenet, treasurenet_indexer, treasurenet_rocksdb, geth):
    """
    run on both treasurenet and geth
    """

...
from .network import CosmosChain, Treasurenet
from .utils import (
    ADDRS,
    KEYS,
    OSMOSIS_POOLS,
...
def ibc(request, tmp_path_factory):
    ...
    network = prepare_network(path, name, [treasurenet_build, "osmosis"])
    yield from network


def test_osmosis_swap(ibc):
    assert_ready(ibc)
    treasurenet: Treasurenet = ibc.chains["treasurenet"]
    osmosis: CosmosChain = ibc.chains["osmosis"]

    treasurenet_addr = ADDRS["signer2"]
    ...
    # --------- Transfer Osmo to Treasurenet
    transfer_osmo_to_treasurenet(ibc, osmosis_addr, treasurenet_addr)

    # define TWAP parameters
    testSlippagePercentage = 20
    testWindowSeconds = 10

    # --------- Swap Osmo to Treasurenet
    w3 = treasurenet.w3
    pc = get_precompile_contract(w3, "IOsmosisOutpost")
    treasurenet_gas_price = w3.eth.gas_price
    swap_params = {
        "channelID": "channel-0",
        "xcsContract": xcs_contract,
        "sender": treasurenet_addr,
        "input": WEVMOS_ADDRESS,
        "output": WOSMO_ADDRESS,
        "amount": amt,
        "slippagePercentage": testSlippagePercentage,
        "windowSeconds": testWindowSeconds,
        "swapReceiver": eth_to_bech32(treasurenet_addr),
    }
    tx = pc.functions.swap(swap_params).build_transaction(
        {"from": treasurenet_addr, "gasPrice": treasurenet_gas_price, "gas": 30000000}
    )
    gas_estimation = treasurenet.w3.eth.estimate_gas(tx)
    ...
    # function transfer_osmo_to_treasurenet
    initial_erc20_balance = 200

    def check_erc20_balance_change():
        nonlocal new_erc20_balance
        new_erc20_balance = erc20_balance(w3, WOSMO_ADDRESS, treasurenet_addr)
...
def setup_osmos_chains(ibc):
    # Send Treasurenet to Osmosis to be able to set up pools
    send_treasurenet_to_osmos(ibc)

    osmosis = ibc.chains["osmosis"]
    osmosis_cli = osmosis.cosmos_cli()
    osmosis_addr = osmosis_cli.address("signer2")

    # create treasurenet <> osmo pool
    pool_id = create_osmosis_pool(
        osmosis_cli, osmosis_addr, OSMOSIS_POOLS["Treasurenet_Osmo"]
    )
    ...
    contracts_to_store = {
        ...
        "CrosschainSwap": {
            "get_instantiate_params": lambda x, y, z: f'{{"governor":"{x}", "swap_contract": "{y}", "channels": [["treasurenet","{z}"]]}}',  # noqa: 501 - ignore line length lint
        },
    }
    ...
    # in the router one execute function `set_route` to have a route for treasurenet within the swap router contract
    # set input 'aunit', output 'uosmo' route
    set_swap_route(
        osmosis_cli, osmosis_addr, swap_contract_addr, pool_id, TREASURENET_IBC_DENOM, "uosmo"
    )

    return xcs_contract


def send_treasurenet_to_osmos(ibc):
    src_chain = ibc.chains["treasurenet"]
    ...
    old_dst_balance = get_balance(dst_chain, dst_addr, TREASURENET_IBC_DENOM)

    pc = get_precompile_contract(src_chain.w3, "ICS20I")
    treasurenet_gas_price = src_chain.w3.eth.gas_price
    ...
    tx_hash = pc.functions.transfer(
        ...
        0,
        "",
    ).transact({"from": ADDRS["signer2"], "gasPrice": treasurenet_gas_price})
    ...
    def check_balance_change():
        ...
        new_dst_balance = get_balance(dst_chain, dst_addr, TREASURENET_IBC_DENOM)
...
def transfer_osmo_to_treasurenet(ibc, src_addr, dst_addr):
    src_chain: CosmosChain = ibc.chains["osmosis"]
    dst_chain: Treasurenet = ibc.chains["treasurenet"]

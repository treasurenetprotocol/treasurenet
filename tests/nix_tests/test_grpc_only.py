...
from .network import Treasurenet, create_snapshots_dir, setup_custom_treasurenet
from .utils import (
    CONTRACTS,
    decode_bech32,
    deploy_contract,
...
def grpc_eth_call(port: int, args: dict, chain_id=None, proposer_address=None):
    ...
    return requests.get(f"http://localhost:{port}/treasurenet/evm/v1/eth_call", params).json()


def test_grpc_mode(treasurenet_cluster: Treasurenet):
    """
    - restart a fullnode in grpc-only mode
    - test the grpc queries all works
    """
    w3 = treasurenet_cluster.w3
    ...
    api_port = ports.api_port(treasurenet_cluster.base_port(1))
    ...
    for i in range(2):
        wait_for_block(treasurenet_cluster.cosmos_cli(i), 1)
    supervisorctl(treasurenet_cluster.base_dir / "../tasks.ini", "stop", "treasurenet_5005-1-node1")

    # run grpc-only mode directly with existing chain state
    with (treasurenet_cluster.base_dir / "node1.log").open("a") as logfile:
        proc = subprocess.Popen(
            [
                treasurenet_cluster.chain_binary,
                "start",
                "--grpc-only",
                "--home",
                treasurenet_cluster.base_dir / "node1",
            ],
            stdout=logfile,
        ...
        try:
            ...
            grpc_port = ports.grpc_port(treasurenet_cluster.base_port(1))
            ...
            addr = treasurenet_cluster.cosmos_cli(0).consensus_address()

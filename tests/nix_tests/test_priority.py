...
def custom_treasurenet(tmp_path_factory):
    ...
    yield from setup_treasurenet(path, 26800, long_timeout_commit=True)
...
def custom_treasurenet_rocksdb(tmp_path_factory):
    ...
    # run with long timeout commit to ensure all
    # txs are included in the same block
    yield from setup_treasurenet_rocksdb(path, 26810, long_timeout_commit=True)
...
def test_priority(treasurenet_cluster):
    """
...
def test_native_tx_priority(treasurenet_cluster):
    ...
    test_cases = [
        {
            ...
            "amount": "1000aunit",
            "gas_prices": f"{base_fee + PRIORITY_REDUCTION * 600000}aunit",
        ...
        },
        {
            ...
            "amount": "1000aunit",
            "gas_prices": f"{base_fee + PRIORITY_REDUCTION * 400000}aunit",
    ...
    for tc in test_cases:
        ...
        gas_price = int(tc["gas_prices"].removesuffix("aunit"))
